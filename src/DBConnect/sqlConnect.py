...
class sqlConn(object):
    ...
    #
    @staticmethod
    def connect(dbname=dbname, user=user, password=password, host=ip, port=port):
        sqlConn.conn = psycopg2.connect(
    ...
    def getStudent(name='', surname='', patronymic=''):
        ...
        for row in sqlConn.cursor:
            student = {'id': row[0], 'firstname': row[1], 'lastname': row[2], 'patronymic': row[3],
                       'birth': row[4], 'email': row[5], 'telephone': row[6], 'faculty': row[7], 'course': row[8],
                       'room': row[9], 'sex': row[11]}
    ...
    def addStudent(Name, Surname, Patronymic, bd, mail, telephone, faculty, course, room, sex):
        record = (Surname, Name, Patronymic, bd, mail, telephone, faculty, course, room, sex)
        sqlConn.cursor.execute(
            'INSERT INTO students (surname,name,patronymic,birth,email,phone,faculty,course,room,sex) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ',
            record)
        sqlConn.conn.commit()
    ...
    @staticmethod
    def getSudentByID(id):
        student ={}
        sqlConn.cursor.execute('''SELECT * FROM students WHERE
        		 id = %(id)s ''',
                               {'id': id})
        for row in sqlConn.cursor:
            student = {'id': row[0], 'name': row[1], 'surname': row[2], 'patronymic': row[3]}
        return student
